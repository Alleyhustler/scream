 ...
 * 
 * Checks for the existence of the `countryCode` and `stateCode` geo cookies (set previously or set by CDN).
 * If the cookies do not exist, it will use the ME Token service to get the country and state codes via HTTP call and
 * write the cookies for use by the WM consent library
 */

window.WM.UserGeo = window.WM.UserGeo || {};

WM.UserGeo.checkGeoCookies = function checkGeoCookie() {
    return this.checkCookie('countryCode') && this.checkCookie('stateCode');
}

WM.UserGeo.checkCookie = function checkCookie(name) {
...
WM.UserGeo.setGeoCookie = function setGeoCookie(countryCode, stateCode) {
    document.cookie = "countryCode=" + countryCode + ";path=/";
    document.cookie = "stateCode=" + stateCode + ";path=/";
}

if (!WM.UserGeo.checkGeoCookies()) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var respObj = JSON.parse(xhttp.responseText);
            WM.UserGeo.setGeoCookie(respObj.country_alpha2, respObj.states[0].state);
        }
    }
